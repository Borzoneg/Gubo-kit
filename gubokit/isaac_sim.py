...
class BasicSetup(Node):
    ...
    def setup_world(self):
        ...
        self.robot = SimRobot("/World/ur5e", os.environ['FLUENTLY_WS_PATH'] + "/urdf/ur5e.urdf", tcp_frame_urdf="tool0", n_gripper_articulation = 2)
        self.robot.subscribe_to_topic('joint_states') # use a JointPublisher to send joint configuration to the robot
        self.target_pose = self.world.scene.add(XFormPrim(prim_path="/World/target", name="target", orientation=rot_utils.euler_angles_to_quats([0, np.pi, 0])))
